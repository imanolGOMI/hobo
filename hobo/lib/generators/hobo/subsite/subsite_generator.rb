...
module Hobo
  module Generators
    ...
    # `rails generate hobo:subsite <name> [--administrators-only] [--theme=]`
    #
    # A part of the application with its own controllers over the same models --
    # an administration, a staff area, a customer portal.
    #
    ...
    class SubsiteGenerator < Rails::Generators::Base
      ...
      argument :subsite, :type => :string, :desc => "Nombre del subsitio"

      class_option :administrators_only, :type => :boolean, :default => false,
                   :desc => "Solo para administradores (anade el campo si no esta)"
      ...
      # question: an administration is a different kind of place.
      def choose_a_theme
      ...
      # question for the application, not a list written here.
      ...
      # user generators write it when the site is invite-only; here it may not
      # be there yet.
      def ensure_the_administrator_field
        return unless administrators_only?
        ...
        inject_into_class user, "User", indent(<<~'RUBY', 2)
          ...
          # Who may see the subsite. `add_fields` and not `fields do`: the table
          # is Rails', and Hobo only adds this column to it.
          add_fields do
      ...
      def say_what_happened
        lines = ["", "El subsitio esta en /#{subsite}."]
        lines << "Es para administradores." if administrators_only?
      ...
      def administrators_only? = options[:administrators_only]
      ...
      # front page `portada` the subsite got a controller for the front page.
      def models
