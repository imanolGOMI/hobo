...
module FormSpike
  ...
  module Helper
    ...
    #
    # `current_user` used to be stubbed here too, and it is gone.
    # `Rapid::Tag.include` reaches **every** tag, not just this port, so once
    # layer 7 made one gem of five the stub answered for the catalogue as well:
    # `current_user` became nil for everybody and the navigation bar stopped
    # saying who you were -- in another suite's test. The catalogue provides it
    # now, and outside a request it answers what this was pretending: nobody.
    #
    # `authenticity_token` stays: nothing else defines an instance method by
    # that name. The catalogue asks `HoboRapid.authenticity_token`, which is the
    # module, not this.
...
# `<error-messages>` used to be defined here too, and it is gone: the catalogue
...
Rapid.define(:spike_input, :attrs => [:name]) do
    ...
    comma_split(attributes[:fields]).each do |field|
      with_scope(:field_name => field) do
        tag("div", { :class => "field" }, :"#{field}_field") do
          ...
          param(:"#{field}_view") { call_tag(:spike_input, { :name => field }, :as => :"#{field}_tag") }
        end
      end
    end
  end
